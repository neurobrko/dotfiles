...
- The provider auto-detects your environment (tmux ➝ tmux provider, Kitty ➝ kitty provider opening its own tab, otherwise snacks).
...
1. Ensure the `opencode` CLI is on your `$PATH` and run it with `--port` (providers do this automatically). Kitty users need `allow_remote_control yes` and `listen_on` configured (the provider launches OpenCode in a new Kitty tab).
